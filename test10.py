...
small_train = dataset["train"].shuffle(seed=42).select(range(2500))
small_eval = dataset["test"].shuffle(seed=42).select(range(1200))
...
def tokenize_function(examples):
    texts = [
        f"{t} {q}"
        for t, q in zip(examples["question_title"], examples["question_content"])
    ]
...
small_train = small_train.remove_columns(["question_title", "question_content"])
small_eval = small_eval.remove_columns(["question_title", "question_content"])
